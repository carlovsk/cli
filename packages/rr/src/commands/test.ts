...
export default class Test extends Command {
  static override description = 'Run unit tests on selected services';

  static override examples = [
    'rr test',
    'rr test --services billing,store',
    'rr test -s billing,store',
    'rr test --all',
    'rr test --all --parallel',
  ];
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags.all) {
        ...
        selectedServices = await selectServices('Select services to test:');
      }
